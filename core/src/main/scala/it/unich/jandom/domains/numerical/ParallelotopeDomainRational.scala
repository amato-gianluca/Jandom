...
import it.unich.jandom.utils.breeze.Rational
 ...
 */
class ParallelotopeDomainRational private (favorAxes: Boolean) extends NumericalDomain {
  ...
  def apply(low: DenseVector[Rational], A: DenseMatrix[Rational], high: DenseVector[Rational]): Property = {
  ...
  def top(n: Int): Property = {
    val low = DenseVector.fill(n)(Rational.NegativeInfinity)
    val high = DenseVector.fill(n)(Rational.PositiveInfinity)
  ...
  def bottom(n: Int): Property = {
    val low = DenseVector.fill(n)(Rational.one)
    val high = DenseVector.fill(n)(Rational.zero)
    val A = DenseMatrix.eye[Rational](n)
   ...
   */
  private def extremalsInBox(lf: DenseVector[Rational], low: DenseVector[Rational], high: DenseVector[Rational]): (Rational, Rational) = {
    var minc = Rational.zero
    var maxc = Rational.one
   ...
   */
  private def pivoting(m: IndexedSeq[DenseVector[Rational]]): Seq[Int] = {
    val dimension = m(0).length
    var indexes = Seq[Int]()
    var pivots = Seq[(DenseVector[Rational], Int)]()
    var i = 0
  ...
  final class Property(
    val isEmpty: Boolean,
    val low: DenseVector[Rational],
    val A: DenseMatrix[Rational],
    val high: DenseVector[Rational])
    ...
    type Domain = ParallelotopeDomainRational

    def domain = ParallelotopeDomainRational.this
    ...
    def widening(that: Property): Property = {
      ...
      for (i <- 0 to dimension - 1) {
        if (thatRotated.low(i) < low(i)) newlow(i) = Rational.NegativeInfinity
        if (thatRotated.high(i) > high(i)) newhigh(i) = Rational.PositiveInfinity
      }
    ...
    def union(that: Property): Property = {
       ...
       */
      type PrioritizedConstraint = (DenseVector[Rational], Rational, Rational, Int)
       ...
       */
      def newConstraint(vi: DenseVector[Rational], vj: DenseVector[Rational], min1i: Rational, min2i: Rational, min1j: Rational, min2j: Rational): Option[DenseVector[Rational]] = {
    ...
    def linearAssignment(n: Int, lf: LinearForm[Double]): Property = linearAssignment_m(n, LinearForm(lf.coeffs map { Rational(_) }: _*) )
     ...
     * @throws $ILLEGAL
     */
    def linearAssignment_m(n: Int, lf: LinearForm[Rational]): Property = {
    ...
    private def dotprod(x: DenseVector[Rational], y: DenseVector[Rational], remove: Int = -1): Rational = {
      var sum: Rational = 0
      for (i <- 0 until x.length if i != remove if x(i) != 0) sum = sum + x(i) * y(i)
      sum
    }

    def linearInequality(lf: LinearForm[Double]): Property = linearInequality_m( LinearForm(lf.coeffs map { Rational(_) }: _*) )
     ...
     * @throws ILLEGAL
     */
    def linearInequality_m(lf: LinearForm[Rational]): Property = {
    ...
    def linearDisequality(lf: LinearForm[Double]): Property = linearDisequality_m( LinearForm(lf.coeffs map { Rational(_) }: _*) )
     ...
     * @throws $ILLEGAL
     */
    def linearDisequality_m(lf: LinearForm[Rational]): Property = {
      val tcoeff = lf.homcoeffs
      val known = lf.known
    ...
    def nonDeterministicAssignment(n: Int): Property = {
      ...
      newlow(pivot) = Rational.NegativeInfinity
      newhigh(pivot) = Rational.PositiveInfinity
      new Property(false, newlow, newA, newhigh)
    }

    def addVariable(): Property = {
      if (isEmpty)
        ParallelotopeDomainRational.this.bottom(A.rows + 1)
      else {
        ...
        val newlow = DenseVector.vertcat(low, DenseVector(Rational.NegativeInfinity))
        val newhigh = DenseVector.vertcat(high, DenseVector(Rational.PositiveInfinity))
        new Property(false, newlow, newA, newhigh)
      }
    }
    ...
    def delVariable(n: Int): Property = {
      ...
      if (isEmpty)
        ...
        (set1 ++ set2).foldLeft(ParallelotopeDomainRational.this.top(A.rows - 1)) { (p, lf) => p.linearInequality_m(lf) }
      }
    }
    ...
    def linearEvaluation(lf: LinearForm[Double]): (Double, Double) = {
       val le =linearEvaluation_m(LinearForm(lf.coeffs map { Rational(_) }: _*))
     ...
     */
    def linearEvaluation_m(lf: LinearForm[Rational]): (Rational, Rational) = {
    ...
    def minimize_m(lf: LinearForm[Rational]) = linearEvaluation_m(lf)._1

    def maximize_m(lf: LinearForm[Rational]) = linearEvaluation_m(lf)._2

    def frequency_m(lf: LinearForm[Rational]) = {
    ...
    def bottom = ParallelotopeDomainRational.this.bottom(dimension)

    def top = ParallelotopeDomainRational.this.top(dimension)
    ...
    def rotate(Aprime: DenseMatrix[Rational]): Property = {
      ...
      val newlow = DenseVector.zeros[Rational](dimension)
      val newhigh = DenseVector.zeros[Rational](dimension)
      B.foreachPair {
        case ((i, j), v) =>
          if (v > 0) {
            newlow(i) += v * low(j)
      ...
      def lfToString(lf: DenseVector[Rational]): String = {
        ...
        for (index <- 0 until dimension) {
          ...
          val term = coeff match {
            ...
            case Rational.minusone => "-" + vars(index)
 ...
 */
object ParallelotopeDomainRational {
  private lazy val standard = new ParallelotopeDomainRational(false) with CachedTopBottom
  private lazy val favoring = new ParallelotopeDomainRational(true) with CachedTopBottom
